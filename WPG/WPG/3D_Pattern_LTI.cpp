//
//	3D_Pattern_LTI.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
#include "QuadProg.h"
#include <chrono>
...
#define TIME_DSP			double(0.4)			//	Time duration for Double Support Phase : dT의 정수배
#define TIME_SSP			double(1.6)			//	Time duration for Single Support Phase : dT의 정수배
#define TIME_S2W			double(2.0)			//	Time duration for Stance to Walk : ~ TIME_PREVIEW
...
#define TOE_OUT             double(10)			//  toe out 각도

#define CoM_H               double(0.65)		//	CoM - ZMP : 현재는 constant
#define G_GRAVITY			double(9.81)		//	중력가속도
#define ALPHA_m             (G_GRAVITY * 0.5)
#define ALPHA_M             (G_GRAVITY * 1.5)
#define BETA_m              double(0.5)
#define BETA_M              double(0.8)

#define m_G                 double(50.0)		//	무게 : 약 50kg?
#define GAMMA               (BETA_m / ALPHA_M + BETA_M / ALPHA_m) / 2          //  감마 평균
#define ETA                 (1 / m_G * ALPHA_M + 1 / m_G * ALPHA_m) / 2        //  에타 평균

#define FOOT_WIDTH			double(0.1)			//	발볼의 너비
#define FOOT_LENGTH			double(0.2)			//	발 길이
#define STEP_LENGTH			double(0.65)		//	스텝 길이
#define STEP_WIDTH			double(0.2)			//	양발 중심간의 거리

#define THETA_STEP_START    int(3)				//  몇번째 걸음에서 회전을 시작하는가 (THETA_STEP_START <= NO_STEP)
#define THETA_STEP          int(0)				//  회전 하는 걸음의 개수 (THETA_STEP_START + THETA_STEP < NO_STEP)
#define THETA		    	double(15.0)		//	발목의 회전 각도
...
#define FRIC_START_TIME     double(8.0)			//  Low friction star time :' 입력시간 + (PREVIEW_DIM * dT)'부터 적용됨
#define FRIC_TIME_LENGTH    double(0.0)			//	Low friction time length
...
void Set_ZMP_Reference(double* xZMP_ref, double* yZMP_ref, double* zZMP_ref, double* x_foot, double* y_foot);
void Get_ZMP_Reference(const unsigned index, const unsigned nDim, double* ZMP_ref, CstVector<PREVIEW_DIM>& preview_ZMP_ref);

void Predict_friction(double* C_f, double fric_start_time, double fric_time_length, double friction, const unsigned DIM_TOTAL);
void Get_Friction(const unsigned k, const unsigned DIM_TOTAL, double* friction, CstVector<PREVIEW_DIM>& Friction);
...
void Get_z_bound(const unsigned k, const unsigned DIM_TOTAL, double* z_Bound, CstVector<PREVIEW_DIM>& z_bound);

CstMatrix<PREVIEW_DIM, STATE_DIM> P_ps, P_vs, P_zs, P_as;
CstMatrix<PREVIEW_DIM, STATE_ANG> H_ps, H_vs;
CstMatrix<PREVIEW_DIM, PREVIEW_DIM> P_pu, P_vu, P_zu, P_au, H_pu, H_vu;
...
int main()
{
	...
	double xZMP_ref[MAX_ARRAY], yZMP_ref[MAX_ARRAY], zZMP_ref[MAX_ARRAY];
	double x_foot[4 * (NO_STEPS + 3)], y_foot[4 * (NO_STEPS + 3)];
	double C_fric[MAX_ARRAY];
	//	double z_Bound[MAX_ARRAY];

	CstMatrix<PREVIEW_DIM, PREVIEW_DIM > E_N, C_THETA, S_THETA, C_f_matrix;
	CstMatrix<PREVIEW_DIM * 4, PREVIEW_DIM * 2> THETA_matrix;
	CstMatrix<PREVIEW_DIM * 2, PREVIEW_DIM * 6> D_MM, D_mM, D_Mm, D_mm;
	CstMatrix<PREVIEW_DIM * 4, PREVIEW_DIM> K_z;
	CstMatrix<PREVIEW_DIM * 4, PREVIEW_DIM * 6> K_fric;
	CstMatrix<PREVIEW_DIM * 16, PREVIEW_DIM * 6> K_zmp;
	CstMatrix<PREVIEW_DIM * 6, PREVIEW_DIM * 6> E_Mat, Q_Mat, Q_Mat_x, Q_Mat_y, A_x, B_x, C_x, A_y, B_y, C_y;
	CstMatrix<PREVIEW_DIM * 24, PREVIEW_DIM * 6> CI;
	CstMatrix<PREVIEW_DIM, PREVIEW_DIM * 6> S_ux, S_uy, S_uz, S_vx, S_vy, S_vz, U_x, U_y;

	CstVector<PREVIEW_DIM> preview_ZMP_ref_x, preview_ZMP_ref_y, preview_ZMP_ref_z, preview_CoM_ref_x, preview_CoM_ref_y, preview_CoM_velocity_ref_x, preview_CoM_velocity_ref_y;
	CstVector<PREVIEW_DIM> g_c, u_x, u_y, u_z, nu_x, nu_y, nu_z, C_f, alpha_M, alpha_m, beta_M, beta_m;
	CstVector<PREVIEW_DIM > l_M, l_m, w_M, w_m;
	CstVector<PREVIEW_DIM * 2> ci0_d_zmp_mm, ci0_d_zmp_mM, ci0_d_zmp_Mm, ci0_d_zmp_MM;
	CstVector<PREVIEW_DIM * 4> d_fric, d_z, Bound;
	CstVector<PREVIEW_DIM * 16> d_zmp;
	CstVector<PREVIEW_DIM * 6> g0, solution, E_x, F_x, G_x, E_y, F_y, G_y, I_z;
	CstVector<PREVIEW_DIM * 24> ci0;

	CstVector<PREVIEW_DIM> z_bound;

	//CstVector<PREVIEW_DIM> tmp_state_y, tmp_state_x, tmp_velocity_y, tmp_velocity_x, tmp_acceleration_y, tmp_acceleration_x;
	...
	Set_ZMP_Reference(xZMP_ref, yZMP_ref, zZMP_ref, x_foot, y_foot);
	...
	//Set_z_Bound(DIM_SSP, DIM_DSP, DIM_S2W, DIM_TOTAL, z_Bound);
	...
	do {
		...
		E_Mat.setBlockMatrix(PREVIEW_DIM, PREVIEW_DIM, ALPHA2 * E_N);
		E_Mat.setBlockMatrix(PREVIEW_DIM * 2, PREVIEW_DIM * 2, ALPHA3 * E_N);
		E_Mat.setBlockMatrix(PREVIEW_DIM * 3, PREVIEW_DIM * 3, ALPHA4 * E_N);
		E_Mat.setBlockMatrix(PREVIEW_DIM * 4, PREVIEW_DIM * 4, ALPHA5 * E_N);
		E_Mat.setBlockMatrix(PREVIEW_DIM * 5, PREVIEW_DIM * 5, ALPHA6 * E_N);

		S_ux.setBlockMatrix(0, 0, E_N);
		S_uy.setBlockMatrix(0, PREVIEW_DIM, E_N);
		S_uz.setBlockMatrix(0, PREVIEW_DIM * 2, E_N);
		S_vx.setBlockMatrix(0, PREVIEW_DIM * 3, E_N);
		S_vy.setBlockMatrix(0, PREVIEW_DIM * 4, E_N);
		S_vz.setBlockMatrix(0, PREVIEW_DIM * 5, E_N);
		...
		//	LTI 문제의 경우 : 한번만 풀면 됨!
		//QuadProg::Cholesky_Decomposition(Q_Mat_x);
		//QuadProg::Cholesky_Decomposition(Q_Mat_y);
		//QuadProgPP::Cholesky_Decomposition(Q_Mat);
		...
		C_THETA.setDiagonal(cos(D2R(THETA)));
		S_THETA.setDiagonal(sin(D2R(THETA)));
		...
		THETA_matrix.setBlockMatrix(0, PREVIEW_DIM, S_THETA);
		THETA_matrix.setBlockMatrix(PREVIEW_DIM, 0, -C_THETA);
		THETA_matrix.setBlockMatrix(PREVIEW_DIM, PREVIEW_DIM, -S_THETA);
		THETA_matrix.setBlockMatrix(PREVIEW_DIM * 2, 0, -S_THETA);
		THETA_matrix.setBlockMatrix(PREVIEW_DIM * 2, PREVIEW_DIM, C_THETA);
		THETA_matrix.setBlockMatrix(PREVIEW_DIM * 3, 0, S_THETA);
		THETA_matrix.setBlockMatrix(PREVIEW_DIM * 3, PREVIEW_DIM, -C_THETA);

		D_mm.setBlockMatrix(0, 0, (P_pu - (BETA_m / ALPHA_M) * P_au) * S_ux - (1 / (m_G * ALPHA_M)) * H_vu * S_vy);
		D_mm.setBlockMatrix(PREVIEW_DIM, 0, (P_pu - (BETA_m / ALPHA_M) * P_au)* S_uy + (1 / (m_G * ALPHA_M)) * H_vu * S_vx);
		...
		K_zmp.setBlockMatrix(PREVIEW_DIM * 4, 0, THETA_matrix* D_mM);
		K_zmp.setBlockMatrix(PREVIEW_DIM * 8, 0, THETA_matrix* D_Mm);
		K_zmp.setBlockMatrix(PREVIEW_DIM * 12, 0, THETA_matrix* D_MM);
		...
		K_fric.setBlockMatrix(PREVIEW_DIM, 0, -P_au * (S_ux + C_f_matrix * S_uz));
		K_fric.setBlockMatrix(PREVIEW_DIM * 2, 0, P_au * (S_uy - C_f_matrix * S_uz));
		K_fric.setBlockMatrix(PREVIEW_DIM * 3, 0, -P_au * (S_uy + C_f_matrix * S_uz));

		K_z.setBlockMatrix(0, 0, P_au);
		K_z.setBlockMatrix(PREVIEW_DIM, 0, -P_au);
		K_z.setBlockMatrix(PREVIEW_DIM * 2, 0, P_pu);
		K_z.setBlockMatrix(PREVIEW_DIM * 3, 0, -P_pu);

		CI.setBlockMatrix(0, 0, K_zmp);
		CI.setBlockMatrix(PREVIEW_DIM * 16, 0, K_fric);
		CI.setBlockMatrix(PREVIEW_DIM * 20, 0, K_z * S_uz);
		...
		Bound.setVector(PREVIEW_DIM, l_m);
		Bound.setVector(PREVIEW_DIM * 2, w_M);
		Bound.setVector(PREVIEW_DIM * 3, w_m);

		ci0_d_zmp_mm.setVector(0, preview_ZMP_ref_x - (P_ps - (BETA_m / ALPHA_M) * P_as) * c_x + (1 / (m_G * ALPHA_M)) * H_vs * zeta_y);
		ci0_d_zmp_mm.setVector(PREVIEW_DIM, preview_ZMP_ref_y - (P_ps - (BETA_m / ALPHA_M) * P_as) * c_y - (1 / (m_G * ALPHA_M)) * H_vs * zeta_x);
		ci0_d_zmp_mM.setVector(0, preview_ZMP_ref_x - (P_ps - (BETA_m / ALPHA_M) * P_as) * c_x + (1 / (m_G * ALPHA_m)) * H_vs * zeta_y);
		ci0_d_zmp_mM.setVector(PREVIEW_DIM, preview_ZMP_ref_y - (P_ps - (BETA_m / ALPHA_M) * P_as) * c_y - (1 / (m_G * ALPHA_m)) * H_vs * zeta_x);
		ci0_d_zmp_Mm.setVector(0, preview_ZMP_ref_x - (P_ps - (BETA_M / ALPHA_m) * P_as) * c_x + (1 / (m_G * ALPHA_M)) * H_vs * zeta_y);
		ci0_d_zmp_Mm.setVector(PREVIEW_DIM, preview_ZMP_ref_y - (P_ps - (BETA_M / ALPHA_m) * P_as) * c_y - (1 / (m_G * ALPHA_M)) * H_vs * zeta_x);
		...
		d_zmp.setVector(PREVIEW_DIM * 4, Bound + THETA_matrix * ci0_d_zmp_mM);
		d_zmp.setVector(PREVIEW_DIM * 8, Bound + THETA_matrix * ci0_d_zmp_Mm);
		d_zmp.setVector(PREVIEW_DIM * 12, Bound + THETA_matrix * ci0_d_zmp_MM);

		g_c.setFill(G_GRAVITY);

		d_fric.setVector(0, C_f_matrix* (g_c + P_as * c_z) - P_as * c_x);
		d_fric.setVector(PREVIEW_DIM, C_f_matrix* (g_c + P_as * c_z) + P_as * c_x);
		d_fric.setVector(PREVIEW_DIM * 2, C_f_matrix* (g_c + P_as * c_z) - P_as * c_y);
		d_fric.setVector(PREVIEW_DIM * 3, C_f_matrix* (g_c + P_as * c_z) + P_as * c_y);

		d_z.setVector(0, alpha_M - P_as * c_z - g_c);
		d_z.setVector(PREVIEW_DIM, -alpha_m + P_as * c_z + g_c);
		d_z.setVector(PREVIEW_DIM * 2, beta_M - P_ps * c_z);
		d_z.setVector(PREVIEW_DIM * 3, -beta_m + P_ps * c_z);


		ci0.setVector(0, d_zmp);
		ci0.setVector(PREVIEW_DIM * 16, d_fric);
		ci0.setVector(PREVIEW_DIM * 20, d_z);


		//	Solve Quadratic Programming
		QuadProgPP::Solve_QuadProg(Q_Mat, g0, (-CI), ci0, solution);

		//	Compute state & ZMP

		for (int i = 0; i < PREVIEW_DIM; i++) {
			u_x[i] = solution[i];
			u_y[i] = solution[PREVIEW_DIM + i];
			u_z[i] = solution[PREVIEW_DIM * 2 + i];

			nu_x[i] = solution[PREVIEW_DIM * 3 + i];
			nu_y[i] = solution[PREVIEW_DIM * 4 + i];
			nu_z[i] = solution[PREVIEW_DIM * 5 + i];
		}
		...
		auto del_time = steady_clock::now() - start_time;
		...
		fprintf_s(fp, "%lf\t %lf\t %lf\t %lf\t %lf\t %lf\t", c_x[0], c_y[0], c_x[1], c_y[1], c_x[2], c_y[2]);
		fprintf_s(fp, "%lf\t %lf\t %lf\t %lf\t %lf\t", zeta_x[0], zeta_y[0], zeta_x[1], zeta_y[1], ((double)elapsed));
...
void Predict_friction(double* C_fric, double fric_start_time, double fric_time_length, double friction, unsigned DIM_TOTAL) {
	...
	for (unsigned i = 0; i <= DIM_TOTAL; i++) {
...
void ComputeDiscreteModel(CstMatrix<nRows, nCols>& A_d, CstVector<nRows>& B_d, CstVector<nCols>& G_d)
{
	A_d[0][0] = 1;			A_d[0][1] = dT;				A_d[0][2] = dT * dT / 2;
	A_d[1][0] = 0;			A_d[1][1] = 1;				A_d[1][2] = dT;
	A_d[2][0] = 0;			A_d[2][1] = 0;				A_d[2][2] = 1;

	B_d[0] = CUBE(dT) / 6;	B_d[1] = SQR(dT) / 2;		B_d[2] = dT;

	G_d[0] = 1;				G_d[1] = 0;					G_d[2] = -height / gravity;
}

template <unsigned nRows, unsigned nCols>
void ComputeDiscreteModel_2(CstMatrix<nRows, nCols>& Alpha_d, CstVector<nRows>& Delta_d)
{
	Alpha_d[0][0] = 1;				Alpha_d[0][1] = dT;
	Alpha_d[1][0] = 0;				Alpha_d[1][1] = 1;

	Delta_d[0] = dT * dT / 2;		Delta_d[1] = dT;
}
...
void Set_ZMP_Reference(double* xZMP_ref, double* yZMP_ref, double* zZMP_ref, double* x_foot, double* y_foot)
{
	...
	for (i = 0; i < MAX_ARRAY; i++) {
		...
		if (i <= DIM_S2W) {
		...
		else if (i > DIM_S2W && i <= DIM_SSP + DIM_S2W) {
		...
		else if (i > DIM_SSP + DIM_S2W && i <= DIM_STEP + DIM_S2W) {
			xZMP_ref[i] = xZMP_ref[DIM_SSP + DIM_S2W] + ((j - DIM_SSP + 1) / (double)DIM_DSP) * l_d;
			yZMP_ref[i] = yZMP_ref[DIM_SSP + DIM_S2W] - pow(-1, (double)residual) * FIRST_FOOT * (STEP_WIDTH + l_s * sin(TOE_OUT * M_PI / 180)) * ((j - DIM_SSP + 1) / (double)DIM_DSP);
			zZMP_ref[i] = CoM_H - 0.05 * cos(2 * M_PI * j / (double)DIM_SSP);
		...
		else if (i > DIM_STEP + DIM_S2W && i <= (NO_STEPS)* DIM_STEP + DIM_S2W) {
			if (j % DIM_STEP == 0) {
				k++;
			}

			if (i > THETA_STEP_START * DIM_STEP + DIM_S2W && i <= (THETA_STEP_START + THETA_STEP) * DIM_STEP + DIM_S2W && j % DIM_STEP == 0) {
				theta = theta + THETA;
			}
			if (i > (THETA_STEP_START - 1) * DIM_STEP + DIM_S2W && i <= (THETA_STEP_START + THETA_STEP - 1) * DIM_STEP + DIM_S2W && j % DIM_STEP == 0) {
				theta_2 = theta + THETA;

			}


			if (i > k * DIM_STEP + DIM_S2W && i <= k * (DIM_STEP)+DIM_SSP + DIM_S2W) {
				xZMP_ref[i] = xZMP_ref[k * DIM_STEP + DIM_S2W] + ((j % DIM_STEP + 1) / (double)DIM_SSP) * l_s * cos(theta * M_PI / 180);
				yZMP_ref[i] = yZMP_ref[k * DIM_STEP + DIM_S2W] + ((j % DIM_STEP + 1) / (double)DIM_SSP) * (FIRST_FOOT * pow(-1, (double)residual) * l_s * sin(TOE_OUT * M_PI / 180) + l_s * sin(theta * M_PI / 180));
				...
				xZMP_ref[i] = xZMP_ref[k * (DIM_STEP)+DIM_SSP + DIM_S2W] + (((j % DIM_STEP) - DIM_SSP + 1) / (double)DIM_DSP) * (l_d * cos(theta * M_PI / 180) + FIRST_FOOT * pow(-1, (double)residual) * STEP_WIDTH * sin(theta * 2 * M_PI / 360));
				yZMP_ref[i] = yZMP_ref[k * (DIM_STEP)+DIM_SSP + DIM_S2W] - FIRST_FOOT * pow(-1, (double)residual) * (((j % DIM_STEP) - DIM_SSP + 1) / (double)DIM_DSP) * ((STEP_WIDTH * cos(theta * M_PI / 180) - FIRST_FOOT * pow(-1, (double)residual) * l_d * sin(theta * M_PI / 180)) + l_s * sin(TOE_OUT * M_PI / 180));
		...
		else if (i > (NO_STEPS)* DIM_STEP + DIM_S2W && i <= DIM_SSP + (NO_STEPS)* DIM_STEP + DIM_S2W) {
			xZMP_ref[i] = xZMP_ref[(NO_STEPS)* DIM_STEP + DIM_S2W] + (((j % DIM_STEP) + 1) / (double)DIM_SSP) * (l_s / 2) * cos(theta * 2 * M_PI / 360);
			yZMP_ref[i] = yZMP_ref[(NO_STEPS)* DIM_STEP + DIM_S2W] + (((j % DIM_STEP) + 1) / (double)DIM_SSP) * ((l_s / 2) * sin(theta * 2 * M_PI / 360) + FIRST_FOOT * pow(-1, (double)residual) * l_s * sin(TOE_OUT * M_PI / 180));
			if ((j % DIM_STEP) < DIM_SSP / 4) {
				zZMP_ref[i] = CoM_H - 0.05 * cos(2 * M_PI * (j % DIM_STEP + DIM_DSP) / (double)DIM_SSP);
			}
			else {
			...
			xZMP_ref[i] = xZMP_ref[DIM_SSP + (NO_STEPS) * (DIM_STEP)+DIM_S2W] + foot_info * pow(-1, (double)NO_STEPS) * sin(theta * 2 * M_PI / 360);
			yZMP_ref[i] = yZMP_ref[DIM_SSP + (NO_STEPS) * (DIM_STEP)+DIM_S2W] - pow(-1, (double)NO_STEPS) * (foot_info * cos(theta * 2 * M_PI / 360) + FIRST_FOOT * l_s * sin(TOE_OUT * M_PI / 180));
			zZMP_ref[i] = CoM_H;
		}
	}
	...
	for (int a = 1; a < NO_STEPS; a++) {
		x_foot[4 + 4 * a] = xZMP_ref[a * DIM_STEP + DIM_S2W] - FOOT_WIDTH * sin(b * THETA * M_PI / 180) / 2;
		x_foot[5 + 4 * a] = xZMP_ref[a * DIM_STEP + DIM_S2W] + FOOT_WIDTH * sin(b * THETA * M_PI / 180) / 2;
		x_foot[6 + 4 * a] = xZMP_ref[a * DIM_STEP + DIM_SSP + DIM_S2W] + FOOT_WIDTH * sin(b * THETA * M_PI / 180) / 2;
		x_foot[7 + 4 * a] = xZMP_ref[a * DIM_STEP + DIM_SSP + DIM_S2W] - FOOT_WIDTH * sin(b * THETA * M_PI / 180) / 2;
		if (l_s < FOOT_LENGTH) {
		...
		y_foot[4 + 4 * a] = yZMP_ref[a * DIM_STEP + DIM_S2W] + FOOT_WIDTH * cos(b * THETA * M_PI / 180) / 2;
		y_foot[5 + 4 * a] = yZMP_ref[a * DIM_STEP + DIM_S2W] - FOOT_WIDTH * cos(b * THETA * M_PI / 180) / 2;
		y_foot[6 + 4 * a] = yZMP_ref[a * DIM_STEP + DIM_SSP + DIM_S2W] - FOOT_WIDTH * cos(b * THETA * M_PI / 180) / 2;
		y_foot[7 + 4 * a] = yZMP_ref[a * DIM_STEP + DIM_SSP + DIM_S2W] + FOOT_WIDTH * cos(b * THETA * M_PI / 180) / 2;

		if (a >= THETA_STEP_START - 1 && a < THETA_STEP_START + THETA_STEP - 1) {
			b++;
		}
	}
	x_foot[4 + 4 * NO_STEPS] = xZMP_ref[NO_STEPS * DIM_STEP + DIM_S2W] - FOOT_WIDTH * sin(THETA_STEP * THETA * M_PI / 180) / 2;
	x_foot[5 + 4 * NO_STEPS] = xZMP_ref[NO_STEPS * DIM_STEP + DIM_S2W] + FOOT_WIDTH * sin(THETA_STEP * THETA * M_PI / 180) / 2;
	x_foot[6 + 4 * NO_STEPS] = xZMP_ref[NO_STEPS * DIM_STEP + DIM_SSP + DIM_S2W] + FOOT_WIDTH * sin(THETA_STEP * THETA * M_PI / 180) / 2 + FOOT_LENGTH * cos(THETA_STEP * THETA * M_PI / 180) / 2;
	x_foot[7 + 4 * NO_STEPS] = xZMP_ref[NO_STEPS * DIM_STEP + DIM_SSP + DIM_S2W] - FOOT_WIDTH * sin(THETA_STEP * THETA * M_PI / 180) / 2 + FOOT_LENGTH * cos(THETA_STEP * THETA * M_PI / 180) / 2;
	y_foot[4 + 4 * NO_STEPS] = yZMP_ref[NO_STEPS * DIM_STEP + DIM_S2W] + FOOT_WIDTH * cos(THETA_STEP * THETA * M_PI / 180) / 2;
	y_foot[5 + 4 * NO_STEPS] = yZMP_ref[NO_STEPS * DIM_STEP + DIM_S2W] - FOOT_WIDTH * cos(THETA_STEP * THETA * M_PI / 180) / 2;
	y_foot[6 + 4 * NO_STEPS] = yZMP_ref[NO_STEPS * DIM_STEP + DIM_S2W] - FOOT_WIDTH * cos(THETA_STEP * THETA * M_PI / 180) / 2 + FOOT_LENGTH * sin(THETA_STEP * THETA * M_PI / 180) / 2;
	y_foot[7 + 4 * NO_STEPS] = yZMP_ref[NO_STEPS * DIM_STEP + DIM_S2W] + FOOT_WIDTH * cos(THETA_STEP * THETA * M_PI / 180) / 2 + FOOT_LENGTH * sin(THETA_STEP * THETA * M_PI / 180) / 2;
...
{
	if (k + PREVIEW_DIM <= DIM_TOTAL) {
		memcpy(&Friction[0], &friction[k + 1], sizeof(double) * (PREVIEW_DIM));
	}
}
...
{
	if (k + PREVIEW_DIM <= DIM_TOTAL) {
		memcpy(&z_bound[0], &z_Bound[k + 1], sizeof(double) * (PREVIEW_DIM));
	}
}