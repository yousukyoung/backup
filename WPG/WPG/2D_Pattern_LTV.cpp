//
//	2D_Pattern_LTV.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
#include "QuadProg.h"
#include <chrono>
...
#define TIME_SSP			double(0.6)		//	Time duration for Single Support Phase : dT의 정수배
...
int main()
{
	...
	CstMatrix<PREVIEW_DIM, PREVIEW_DIM> Q_Mat, E_Mat, Transpose_P_zu, ALPHA_trP_zu;
	...
	do {
		...
		//	Set Quadratic cost
		E_Mat.setDiagonal(ALPHA1);
		ALPHA_trP_zu = ALPHA2 * Transpose(P_zu);
		Q_Mat = E_Mat + ALPHA_trP_zu * P_zu;
		...
		//	Solve Quadratic Programming : CASE-2 (LTI/LTV + Inequality Constraints Only!!)
		QuadProgPP::Solve_QuadProg(Q_Mat, g0, CI, ci0, u_y);

		//	Compute state & ZMP
		tmp_state = A_d * c_y + B_d * u_y[0];
		y_zmp = G_d * c_y;

		std::chrono::duration<double> del_time = std::chrono::steady_clock::now() - start_time;
		auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(del_time).count() / 1000.0;
...
void Set_ZMP_Reference(double *xZMP_ref, double *yZMP_ref)
{
	...
	for (i = 0; i < MAX_ARRAY; i++) {
		if (i <= DIM_S2W) {
		...
		else if (i > DIM_S2W && i <= (DIM_DSP + DIM_SSP) + DIM_S2W) {
			j = i - DIM_S2W - 1;
			Quotient = j / (DIM_SSP + DIM_DSP);
			residual = Quotient % 2;
			xZMP_ref[i] = 0;
			yZMP_ref[i] = pow(-1, (double)residual)*foot_info;
		}
		else if (i > (DIM_DSP + DIM_SSP) + DIM_S2W && i <= (NO_STEPS + 1)*(DIM_DSP + DIM_SSP) + DIM_S2W) {
			j = i - DIM_S2W - 1;
			Quotient = j / (DIM_SSP + DIM_DSP);
			residual = Quotient % 2;
			...
			xZMP_ref[i] = xZMP_ref[(NO_STEPS + 1)*(DIM_DSP + DIM_SSP) + DIM_S2W];
			yZMP_ref[i] = 0;
		}
	}
}
